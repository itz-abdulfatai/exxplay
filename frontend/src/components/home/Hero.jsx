...
function Hero() {
  ...
  useEffect(() => {
    const options = {
      strings: [`Gamify your <br> operations`],
      typeSpeed: 100,
      showCursor: false,
      loop: true,
    };
  ...
  return (
    <section
      id="home"
      className=" flex flex-col justify-center gap-14 gradient"
    >
      {/* <span className=" scale-x-[2] transform  inline-block rounded-none mx-10">  o  </span> */}
      <div className=" min-h-32">
        <h1
          className=" tracking-wide text-2xl md:text-6xl max-md:text-center "
          ref={typingEl}
        ></h1>
      </div>
      <div className=" h-[220px] md:h-48  flex justify-between items-center flex-col-reverse md:flex-row ">
        <div>
          <a
            href=" #contact"
            className=" uppercase flex items-center justify-between zp-1 gap-1 py-1 px-1  rounded-full bg-secondary text-primary text-xs font-semibold min-w-[2157x]"
          >
            let&lsquo;s get started{" "}
            <i className="bx bxs-right-top-arrow-circle text-4xl text-accent2"></i>
          </a>
        </div>
        <div className=" max-md:text-center text-xs md:max-w-96 text-right flex flex-col items-center justify-around md:items-end h-full small-grad ">
          ...
          rerum modi iusto earum nemo repellat adipisicing elit. Dolor maiores
          rerum modi iusto
